...
public:
	...
	void solve_equations(std::map<trace_t, T2> & mp) {
		...
		//Unique flow keys
		cout << "hash table size " << hashTableCounter << endl;
		//cout << "not unique N " << flowKeys.size() << endl; 
		std::sort(flowKeys.begin(), flowKeys.end());
		flowKeys.erase(std::unique(flowKeys.begin(), flowKeys.end()), flowKeys.end());
		N = flowKeys.size();
		cout << "cms " << N << endl; 
		cout << "evicted " << evictResult.size() << endl;
	...
	size_t get_memory_usage() {
		...
		std::cout << "Hash Table Size(Byte):" << htSize << "(" << (double)htSize / 1000 / 1000 << "MB)" << std::endl;
		std::cout << "Bloom Filter Size(Byte):" << bfSize << "(" << (double)bfSize / 1000 / 1000 << "MB)" << std::endl;
		std::cout << "CM Sketch Size(Byte):" << cmSize << "(" << (double)cmSize / 1000 / 1000 << "MB)" << std::endl;
		std::cout << "Total Memory(MB):" << ((double)htSize + (double)bfSize + (double)cmSize) / 1000 / 1000 << std::endl;
		std::cout << "------------------------" << std::endl;
