...
template <typename key_type, typename value_type> class HTmap {
        ...
        int max_num_iterations;        // max number of iterations
		int tmax;
		bool victim_flag;
		key_type victim_key;
        ...
        public:
                ...
                void init(int way, int buckets, int hsize,int t);
                virtual ~HTmap();
                void clear();
                void expand();
		...
		//LHS operator[]
                ...
                int get_iterations() {return num_iterations;}
                int get_max_iterations() {return max_num_iterations;}
...
template <typename key_type, typename value_type>
void HTmap<key_type,value_type>::init(int way, int buckets, int hsize,int t)
{
  tmax=t;
...
template <typename T> int myhash(T key, int i, int s) {
    uint64_t val=(uint64_t)bobhash32_run((const char*)&key, sizeof(T),i) % s;
    return (val);
