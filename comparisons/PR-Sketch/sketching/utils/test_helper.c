...
void test() {
	...
	// Test time to estimate all keys
	decoding_test(sketch, tuplekey_set);
	// Get Packets/Bytes ratio
	transmission_test(sketch, tuplekey_vector, bytesum);

	// Get estimators -> insert into estimator keys tracked by sketch + its size estimate
	// also build the hashmap for estimated size by the sketch
	Estimator estimator = get_estimator(sketch, tuplekey_vector);	

	// Flow size distribution difference between groundtruth and sketch estimate
...
Estimator get_estimator(BaseSketch* sketch, vector<tuple_key_t> tuplekey_vector) {
	// here we build the groundtruth
	Estimator estimator = Estimator(tuplekey_vector);

	set<tuple_key_t> tracked_tuplekeys = sketch->get_tuplekeys();
	// iterate over tracked keys (by the sketch), get its estimated size and insert into estimation vector
	// estimation vector is built only by TRACKED keys
...
void flowsize_test(const Estimator& estimator, const char* label) {
	...
	// compute how many flows have a given size -> flow size distribution
	// do for groundtruth
