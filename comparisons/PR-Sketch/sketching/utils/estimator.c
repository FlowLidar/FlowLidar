...
Estimator::Estimator(vector<tuple_key_t> tuple_vector) {
	// build groundtruth from list of packet tuples
	for (uint32_t i = 0; i < tuple_vector.size(); i++) {
		...
		srcport_set.insert(tmp_tuplekey.src_port);
		dstport_set.insert(tmp_tuplekey.dst_port);
		proto_set.insert(tmp_tuplekey.proto);
...
uint32_t Estimator::get_keyspace() const {
	uint32_t keyspace = srcip_set.size() * dstip_set.size() * srcport_set.size() * dstport_set.size() *proto_set.size();
	return keyspace;
}
